...
/// <summary>CadmusTool 测试：插件契约（元数据、RegisterServices / CreateView 冒烟）。</summary>
public class CadmusToolTests
{
    [Fact]
    public void Metadata_插件元数据_id与显示名符合约定()
    {
        var tool = new CadmusTool();

        Assert.Equal("daedalus.tools.cadmus", tool.Metadata.Id);
    ...
    {
        var tool = new CadmusTool();
        var host = new FakeToolHost(Path.Combine(Path.GetTempPath(), "daedalus-cadmus-tool-" + Guid.NewGuid().ToString("N")));
