...
/// <summary>CadmusSettingsStore 测试：读写往返、缺省容忍、损坏备份恢复（DR-003）。</summary>
public sealed class CadmusSettingsStoreTests : IDisposable
{
    private readonly string _directory = Path.Combine(Path.GetTempPath(), "daedalus-cadmus-tests-" + Guid.NewGuid().ToString("N"));

    public CadmusSettingsStoreTests()
    {
    ...
    {
        var store = new CadmusSettingsStore(_directory);

        CadmusSettingsLoadResult result = await store.LoadAsync();

        Assert.Equal(CadmusSettings.Default, result.Settings);
    ...
    {
        var store = new CadmusSettingsStore(_directory);
        var settings = new CadmusSettings(CadmusSettings.CurrentVersion, "url");
    ...
    public async Task LoadAsync_字段缺失_容忍并以默认补齐()
    {
        ...
        Assert.Equal(new CadmusSettings(1, null), result.Settings);
        Assert.False(result.RecoveredFromCorruption);
    }
    ...
    public async Task LoadAsync_含未知字段_忽略并正常读取()
    {
        var store = new CadmusSettingsStore(_directory);
        string filePath = Path.Combine(_directory, "settings.json");
        await File.WriteAllTextAsync(filePath, """{"version": 1, "lastEncoding": "base64", "futureField": true}""");

        CadmusSettingsLoadResult result = await store.LoadAsync();

        Assert.Equal(new CadmusSettings(1, "base64"), result.Settings);
        Assert.False(result.RecoveredFromCorruption);
    }
}
